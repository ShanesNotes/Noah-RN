...
contract:
  you_will_get:
    - structured systems-based nursing assessment
    - four-layer output with provenance footer
    - bounded clinical flags grounded in provided findings
  you_will_not_get:
    - diagnoses
    - facility-specific policy beyond explicit references
    - fabricated missing assessment findings
  controllable_fields:
    - acuity_level: icu | med-surg | outpatient
    - depth: focused | standard | comprehensive
  use_when:
    - a head-to-toe or systems assessment needs structure
  do_not_use_when:
    - a shift handoff summary is needed instead of assessment detail
...
Accept the nurse's free-text clinical narrative. This can be dense ICU-style, casual med-surg, partial, or any combination. Do not ask clarifying questions about the input. Parse what is provided.

### Step 2: Infer Acuity

Determine the care setting from input content using the acuity convention in `packages/workflows/CONVENTIONS.md`. Do not ask the nurse to specify.
...
Map the nurse's input to the 15 assessment systems below. Preserve the nurse's clinical language -- do not sanitize abbreviations or rewrite their phrasing. Standardize formatting only.
...
> structure used in acute/critical care settings (ANA Standards of Practice). System ordering
> prioritizes life-threatening findings first (code status, neuro, airway) per ABCDE principles.

### Step 4: Flag Critical Findings

Prefix any finding that warrants immediate clinical attention with `[!]`. This is a documentation marker only. Do NOT prompt the nurse about whether they've taken action.
...
- Adjust gap expectations by acuity: outpatient assessments should NOT flag missing ICU-specific systems.

### Step 6: Render Output

Produce the final structured assessment:
...
- Assessment is a point-in-time snapshot. Trends over time are more clinically meaningful.
