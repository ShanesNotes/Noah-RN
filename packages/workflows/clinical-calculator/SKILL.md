...
contract:
  you_will_get:
    ...
    - component breakdown and score interpretation
    - bounded bedside flags when thresholds are met
  you_will_not_get:
    - guessed component values
    - facility-specific scoring modifications
    - diagnosis or treatment orders
  controllable_fields:
    - calculator: gcs | nihss | apache2 | wells-pe | wells-dvt | curb65 | braden | rass | cpot | news2
    - input_mode: all_at_once | guided
  use_when:
    - a standardized clinical score needs exact calculation
    - a bedside score needs deterministic verification
  do_not_use_when:
    - medication reference is needed instead of scoring
    - a narrative documentation output is needed instead of a score
