...
10. **The safety layer IS the product.** Orchestration, retrieval, inference are plumbing. Guardrails, confidence calibration, completeness verification, and the "I don't know" response are what make noah-rn trustworthy. Trustworthiness is the only currency at the bedside.
...
14. **Stay in HITL Category II (documentation assistance).** This keeps noah-rn below the SaMD threshold, out of FDA device classification, and keeps the nurse in the driver's seat. Design principle: generate drafts for review, never recommendations that bypass judgment.
...
18. **Knowledge currency is a patient safety issue, not a data quality issue.** A right model on wrong knowledge is the most dangerous failure mode. The `knowledge/` directory isn't supplementary — it's the 33% of clinical facts the model might get wrong from stale training data. Curation quality = patient safety.
...
23. **Test every design decision against the CDS exemption four-part test.** noah-rn stays below SaMD threshold by: (1) not analyzing images, (2) displaying/analyzing medical info, (3) supporting without replacing judgment, (4) enabling independent review. The moment a skill makes autonomous clinical decisions without human review, you've crossed into device territory.
...
26. **Deterministic checks are the irreducible safety floor.** Hard-coded plausibility validation (dosage ranges, drug existence, unit verification) that no prompt injection can bypass. These go in `hooks/` as non-LLM guardrails beneath all model outputs. The Doctronic OxyContin case is the canonical example of what happens without them.
27. **Assume adversarial input at every boundary.** 94.4% prompt injection success rate in clinical LLMs. Every external input (clinical notes, patient text, MCP tool responses, FHIR data) should be validated before reaching skills. Trust nothing; verify everything.
