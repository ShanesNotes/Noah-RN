# Noah RN — Phased Roadmap (v0.2.0)

> Last updated: 2026-04-07
> Purpose: Broad categories of deep work synthesized from 6 Opus PRD docs + existing 25-path exploration map
> Companion: `docs/PENDING-WORK.md` holds detailed interview seeds per exploration path
...
### Category 1: Foundation Migration (Orchestration Runtime)

**What it is.** Migrate noah-rn from being a Claude Code plugin to running on a portable open-source orchestration stack (OpenClaw + NeMo Guardrails + Letta . Claude Code becomes one delivery surface among many, not the product boundary. This is the change that makes model-agnosticism real instead of aspirational.

**Why it matters.** Every other category assumes portability. If you build a PHI pipeline, provider census, and knowledge graph on Claude Code primitives, you've just built more work to unwind later. The Opus agentorchestration doc is emphatic: **OpenClaw consumes SKILL.md files and MCP tools with zero rewriting** — the hardest migration risk (the portability assumption) is testable in a 4-hour prototype.

**Source docs:**
- `notes/agentorchastration.md` (primary, 571 lines — the entire composition strategy)
- `notes/Noah RN Architecture v0.2.0.md` (model-agnosticism invariant)
- `notes/noah-rn-prd-v0.2.0.md` §Vector A

**Sub-work items:**
- **Foundation:** 4-hour prototype experiment from agentorchestration.md — validate SKILL.md + MCP + hook portability to OpenClaw on one skill
- **Phase 0 (parallel run, 1 week):** Install OpenClaw gateway, copy 9 SKILL.md files, configure 6 MCP tools, register bash hooks as OpenClaw plugin, run both Claude Code and OpenClaw against same scenarios
- **Phase 3 (NemoClaw wrapper, 1 week):** Privacy routing (PHI→local Nemotron, non-PHI→Claude), sandboxed execution via OpenShell
- **Phase 4 (cutover, 1 week):** Point Medplum at OpenClaw gateway, decommission Claude Code runtime
- Maps to old exploration path **#17 (Model-Agnostic Harness Design)** — which is now this category's entire purpose

**Phased sub-plan:**
1. **Prove portability** (4 hrs) — prototype experiment, go/no-go gate
2. **Parallel run** (1 week) — both runtimes, compare outputs for parity
3. **Wrap in NemoClaw** (1 week) — privacy routing + sandboxing
4. **Cutover** (1 week) — decommission Claude Code runtime

**Dependencies:** None — this is the root. The prototype experiment is the first thing to do if you ever plan to go portable. If the prototype fails, evaluate Hermes Agent as alternative (4 more hours).

**Estimated session size:** 5-6 weeks of focused work (per agentorchestration.md), but the **go/no-go decision can be made in 4 hours**.

**Risks to watch:**
- NemoClaw is alpha (launched March 2026) — keep as additive, not required
- SKILL.md YAML schema differences between Claude Code and OpenClaw — prototype validates this specifically
- OpenClaw community governance instability — MIT license means fork is always possible
